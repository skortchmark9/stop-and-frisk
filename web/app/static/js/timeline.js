...
function addTimepiece(title, text, link, date){
	...
	var month_num = 2*Math.round(date_obj.getMonth()/2)
	var month_key = month_num + " " + date_obj.getFullYear()
	...
	var num = 0
	if(month_arr == undefined){
		timepiece_months[month_key] = []
		month_arr = []
	}

	num = month_arr.length
	timepiece_months[month_key].push(title)
	...
	tooltip.style.top = start + num*height_difference + "px"
}
